...
parser.add_argument('--modeldir', type=str, default="model/",
                    help="Model path")
parser.add_argument('--epochs', type=int, default=200) #修改
...
def accuracy(output, target):
    ...
    # if ptotal == 0:
    #     return torch.tensor(0.).cuda(args.gpu).cpu(), ncorrect / (
    ...
    if ptotal == 0:
        return torch.tensor(0.).cuda(args.gpu).cpu(), (ncorrect / (batch_size - ptotal) * 100).cpu(), (
                    correct / batch_size * 100).cpu(), ptotal.cpu()
    elif ptotal == batch_size:
        return (pcorrect / ptotal * 100).cpu(), torch.tensor(0.).cuda(args.gpu).cpu(), (
                    correct / batch_size * 100).cpu(), ptotal.cpu()
    else:
        return (pcorrect / ptotal * 100).cpu(), (ncorrect / (batch_size - ptotal) * 100).cpu(), (
                    correct / batch_size * 100).cpu(), ptotal.cpu()

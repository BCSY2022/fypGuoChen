...
class MNIST_Dataset(Dataset):
    ...
    def __init__(self, n_labeled, n_unlabeled, trainX, trainY, testX, testY, type="noisy", split="train", mode="N",
        ...
        self.X, self.Y, self.T, self.oids, self.prior = make_dataset(((trainX, trainY), (testX, testY)), n_labeled,
...
class MNIST_Dataset_FixSample(Dataset):
    ...
    def __init__(self, n_labeled, n_unlabeled, trainX, trainY, testX, testY, type="noisy", split="train", mode="A",
                 ids=None, pn=False, increasing=False, replacement=True, top=0.5, flex=0, pickout=True, seed=None,
                 transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure the image is a PyTorch tensor in the correct shape (1, 28, 28)
        # Assuming image is a numpy array; you might not need this if your images are already tensors
        image = torch.tensor(image, dtype=torch.float)

        # Reshape if necessary (for numpy arrays or if any other format issues)
        if image.dim() == 2:  # if image is (height, width), unsqueeze to add channel dimension
            image = image.unsqueeze(0)  # now image is (1, height, width)

        # Apply transformations
        if self.transform:
            image = self.transform(image)

        return image, label, self.P[index], self.T[index], index, self.result[index]
